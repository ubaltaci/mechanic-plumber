...
module.exports = (request, reply, data) => {
    ...
    Async.each(sortDatas, (modelId, eachCallback) => {
        ...
        return model.findOneAndUpdate({_id: modelId}, {[sortField]: sortDatas.indexOf(modelId)}, eachCallback);
        ...
        return version(request, Inflection.underscore(model.modelName) + "_version", `DELETE | ${request.path}`, () => {
            return reply.redirect(request.pre["baseurl"]);
        });
    });
};