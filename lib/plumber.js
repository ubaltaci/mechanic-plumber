...
function _pipePre(preFilePath) {
    ...
    preFilePath = preFilePath || "hooks/hooks.js";
    const stack = Callsite();
    const requester = stack[1].getFileName();
    const mainPreFilePath = Path.join(Path.dirname(requester), preFilePath);

    let preFile = [];

    try {
        const stat = Fs.statSync(mainPreFilePath);
        if (stat.isFile()) {
            preFile = require(mainPreFilePath);
        }
    }
    catch (e) {
        console.log(Clc.red.bold("Upps. There are not any HOOKS, I guess you're busy something else :("));
...
function _pipeServices(serviceDirectory) {
    ...
    try {
        ...
        console.log(Clc.red.bold("Upps. There are not any SERVICES, I guess you're busy something else :("));
        return {};
    }
