...
function _pipeModels(config) {
    var models = {};
    var mainPluginFolder = Path.join(config.appPath, "plugins");
...
function _pipeModelsInFolder(folder) {
    ...
    var pluginModels = {};
    var modelFsItems = _readDirectoryRecursively(folder, _filterModelFile);
...
function _pipe(plugin, config) {
    ...
    if (PluginPaths.policyFile) {
        _setPolicies();
    }

    if (PluginPaths.viewDirectory) {
        _setViews();
    }
...
function _setViews() {
    Plugin.views({
        ...
        path: PluginPaths.viewDirectory,
        partialsPath: Config.viewSettings.partialsPath,
        helpersPath: Config.viewSettings.helpersPath,
        allowAbsolutePaths: true
    });
}

function _setPolicies() {
    if (Plugin.plugins && Plugin.plugins["hapi-auth-man"] && Plugin.plugins["hapi-auth-man"].policy) {
        Plugin.plugins["hapi-auth-man"].policy(require(PluginPaths.policyFile));
    }
}
...
function _setPluginPaths() {
    ...
    // Set view directory
    PluginPaths.viewDirectory = Path.join(PluginPaths.mainPath, "view");

    if (!Fs.existsSync(PluginPaths.policyFile) || !Fs.statSync(PluginPaths.policyFile).isFile()) {
        PluginPaths.policyFile = null;
    }
    ...
    if (!Fs.existsSync(PluginPaths.viewDirectory) || !Fs.statSync(PluginPaths.viewDirectory).isDirectory()) {
        PluginPaths.viewDirectory = null;
    }
}
