...
module.exports = {
    ...
    pipePolicies: _pipePolicies,
    pipeServerMethods: _pipeServerMethods,
    pipePre: _pipePre
};


/**
 * Pipe policies directory into mechanic.policies
 * object for further usage.
 * Each file exports a single function with signature: function(request, reply)
 * @private
 */

function _pipePolicies() {

    var policies = {};

    var stack = Callsite();
    var requester = stack[1].getFileName();
    var mainPolicyFolder = Path.join(Path.dirname(requester), "utils/policies");

    if (Fs.existsSync(mainPolicyFolder)) {
        var stat = Fs.statSync(mainPolicyFolder);
        if (stat.isDirectory()) {
            var policyFiles = _readDirectoryRecursively(mainPolicyFolder, _filterJavascriptFile);
            policyFiles.forEach(function (policyFile) {
                policies[_trimExtFromFileName(policyFile.name)] = require(policyFile.path);
            });
        }
    }
...
/**
 * Pipe Server method directory into mechanic.pre
 * object for further usage.
 * Each file exports a single object with signature: {fn:function() , options:{}}
 * @private
 */

function _pipeServerMethods() {

    var stack = Callsite();
    var requester = stack[1].getFileName();
    var mainServerMethodFolder = Path.join(Path.dirname(requester), "utils/server-methods");

    if (Fs.existsSync(mainServerMethodFolder)) {
        var stat = Fs.statSync(mainServerMethodFolder);
        if (stat.isDirectory()) {
            var serverMethods = _readDirectoryRecursively(mainServerMethodFolder, _filterJavascriptFile);
            serverMethods.forEach(function (serverMethod) {
                require(serverMethod.path)();
            });
        }
    }
}
...
function _pipePre() {
    ...
    var mainPreFolder = Path.join(Path.dirname(requester), "utils/pre");

    if (Fs.existsSync(mainPreFolder)) {
        var stat = Fs.statSync(mainPreFolder);
        if (stat.isDirectory()) {
            var preFiles = _readDirectoryRecursively(mainPreFolder, _filterJavascriptFile);
            preFiles.forEach(function (preFile) {
                pre[_trimExtFromFileName(preFile.name)] = require(preFile.path);
            });
        }
    }
 ...
 * @private
 */
function _pipeServices() {
    var services = {};
    var stack = Callsite();
    var requester = stack[1].getFileName();

    var mainServiceFolder = Path.join(Path.dirname(requester), "utils/services");
...
function _pipeModelsInFolder(folder) {
    ...
    modelFsItems.forEach(function (modelFsItem) {
        var modelName = _trimExtFromFileName(modelFsItem.name);
    ...
    Routes.forEach(function (route) {
        ...
        if ((typeof config == "string" || config instanceof String) && config.indexOf("@") > 0) {
            ...
            Object.keys(BlueprintRoutes).forEach(function (blueprintRoute) {
                ...
                else if (!relatedController[blueprintRoute] && ((blueprintRoute == "sorting") || (blueprintRoute == "sorted"))) {
                    ;//do nothing
                }
...
function _pipeAllIntoHapi() {
    ...
    routeKeys.forEach(function (routeKey) {
        ...
        if (mechanic.pre) {
            var preNames = Object.keys(mechanic.pre);
            preNames.forEach(function (preName) {
                if (mechanic.pre[preName].availableAfter && HapiRoutes[routeKey].path.indexOf(mechanic.pre[preName].availableAfter) != -1) {
                    ...
                    HapiRoutes[routeKey].config.pre.unshift({
                        method: mechanic.pre[preName].fn,
                        assign: mechanic.pre[preName].assign
                    });
                }
            });
        }
